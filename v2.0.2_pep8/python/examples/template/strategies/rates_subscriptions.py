...
class rates_subscriptions(DWX_ZMQ_Strategy):
    ...
    def stop(self):
        ...
        try:
            # Acquire lock
            self._lock.acquire()
            self._zmq.unsubscribe_all_marketdata()
        ...
        try:
            # Acquire lock
            self._lock.acquire()
            self._zmq.send_trackprices_request([])
            print('\rRemoving symbols list', end='', flush=True)
            sleep(self._delay)
            self._zmq.send_trackrates_request([])
    ...
    def __subscribe_to_rate_feeds(self):
        ...
        1) Setup symbols in Expert Advisor through self._zmq.subscribe_marketdata
        2) Starts price feeding through self._zmq.send_trackrates_request
        """
        if len(self._instruments) > 0:
            ...
            for _instrument in self._instruments:
                try:
                    # Acquire lock
                    self._lock.acquire()
                    self._zmq.subscribe_marketdata(_instrument[0], delimiter=';')
            ...
            try:
                # Acquire lock
                self._lock.acquire()
                self._zmq.send_trackrates_request(self._instruments)

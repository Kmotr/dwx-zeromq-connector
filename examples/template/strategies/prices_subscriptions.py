...
class prices_subscriptions(DWX_ZMQ_Strategy):
    ...
    def __subscribe_to_price_feeds(self):
        ...
        2) Starts price feeding through self._zmq.send_trackprices_request
        """
        if len(self._symbols) > 0:
            ...
            for _symbol in self._symbols:
                try:
                    # Acquire lock
                    self._lock.acquire()
                    self._zmq.subscribe_marketdata(_symbol, delimiter=';')
            ...
            try:
                # Acquire lock
                self._lock.acquire()
                self._zmq.send_trackprices_request(self._symbols)
